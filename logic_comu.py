...
def insereixDataEntregaEnDFFac(df_fac, df_fechas):
...
def netejaTipusDadesDFReal(df_real):
    ...
    df_real["R_ACUMULADO_C"] = pd.to_numeric(df_real["R_ACUMULADO_C"], errors="coerce").fillna(
        0.00
    )
    return df_real
...
def netejaTipusDadesDFPed(df_ped):
    ...
    df_ped["A_ACUMULADO_CP"] = pd.to_numeric(df_ped["A_ACUMULADO_CP"], errors="coerce").fillna(
        0.00
    )
    return df_ped
...
def netejaTipusDadesDFAlb(df_alb):
    ...
    df_alb["A_ACUMULADO_CA"] = pd.to_numeric(df_alb["A_ACUMULADO_CA"], errors="coerce").fillna(
        0.00
    )
    return df_alb
...
def netejaTipusDadesDFFac(df_fac):
    ...
    df_fac["A_ACUMULADO_CF"] = pd.to_numeric(df_fac["A_ACUMULADO_CF"], errors="coerce").fillna(
        0.00
    )
    return df_fac
